...
import {
  ...
  CalendarHeart, ClipboardCheck, UserMinus, ShieldAlert, Stethoscope,
  Moon, Calendar, Hourglass, Repeat, Phone, Palette,
  FileText, CalendarClock, ListChecks, Undo2, ClipboardList, Timer,
  PauseCircle, UserCheck,
} from 'lucide-react'
...
export const TRIGGERS: BlockDef[] = [
  ...
  },
  {
    ...
    description: 'El bot acaba de agendar una cita en esta conversación',
    icon: CalendarHeart,
    color: 'lime',
    group: 'Disparador',
    fields: [],
    defaults: {},
    summary: () => 'Cuando se confirma una cita',
  },
  {
    type: 'appointment_completed',
    kind: 'trigger',
    label: 'Visita completada',
    description: 'El paciente terminó su consulta',
    icon: ClipboardCheck,
    color: 'lime',
    group: 'Disparador',
    fields: [],
    defaults: {},
    summary: () => 'Cuando el paciente sale de la consulta',
  },
  {
    type: 'lead_inactive_days',
    kind: 'trigger',
    label: 'Paciente inactivo',
    description: 'No responde hace N días',
    icon: UserMinus,
    color: 'lime',
    group: 'Disparador',
    fields: [
      { key: 'days', label: 'Días inactivo', type: 'number', placeholder: '7', hint: 'Ideal para reactivación (7d, 14d, 30d).' },
    ],
    defaults: { days: 7 },
    summary: (p) => `Inactivo ≥ ${p['days'] ?? 7} días`,
  },
  {
    type: 'objection_detected',
    kind: 'trigger',
    label: 'Objeción detectada',
    description: 'Dice que está caro, lo piensa, etc.',
    icon: ShieldAlert,
    color: 'lime',
    group: 'Disparador',
    fields: [
      {
        ...
        type: 'select',
        options: [
          { value: 'price',      label: 'Está caro / precio' },
          { value: 'thinking',   label: 'Lo voy a pensar' },
          { value: 'competitor', label: 'Compara con otra clínica' },
          { value: 'no_money',   label: 'No tengo dinero ahora' },
        ],
      },
    ],
    defaults: { kind: 'price' },
    summary: (p) => `Objeción: ${p['kind'] ?? '—'}`,
  },
  {
    type: 'treatment_mentioned',
    kind: 'trigger',
    label: 'Tratamiento mencionado',
    description: 'El paciente nombra un tratamiento de tu lista',
    icon: Stethoscope,
    color: 'lime',
    group: 'Disparador',
    fields: [
      { key: 'treatment', label: 'Tratamiento (opcional)', type: 'text', placeholder: 'ortodoncia, blanqueamiento, implante…', optional: true, hint: 'Vacío = cualquier tratamiento de la lista de la clínica.' },
    ],
    defaults: { treatment: '' },
    summary: (p) => p['treatment'] ? `Menciona "${p['treatment']}"` : 'Menciona algún tratamiento',
  },
]
...
export const CONDITIONS: BlockDef[] = [
  ...
  {
    type: 'quiet_hours_now',
    kind: 'condition',
    label: 'Quiet hours activas',
    description: 'Ahora estamos en horas silenciosas',
    icon: Moon,
    color: 'amber',
    group: 'Condición',
    fields: [],
    defaults: {},
    summary: () => '¿Estamos en quiet hours?',
  },
  {
    type: 'is_weekend',
    kind: 'condition',
    label: 'Es fin de semana',
    description: 'Hoy es sábado o domingo',
    icon: Calendar,
    color: 'amber',
    group: 'Condición',
    fields: [],
    defaults: {},
    summary: () => '¿Es fin de semana?',
  },
  {
    type: 'last_message_age_hours',
    kind: 'condition',
    label: 'Tiempo desde último mensaje',
    description: 'Pasaron ≥ N horas desde el último mensaje',
    icon: Hourglass,
    color: 'amber',
    group: 'Condición',
    fields: [
      { key: 'hours', label: 'Horas', type: 'number', placeholder: '24' },
    ],
    defaults: { hours: 24 },
    summary: (p) => `¿≥ ${p['hours'] ?? 24}h desde último msg?`,
  },
  {
    type: 'times_contacted_above',
    kind: 'condition',
    label: 'Veces contactado',
    description: 'El bot intentó N veces sin respuesta',
    icon: Repeat,
    color: 'amber',
    group: 'Condición',
    fields: [
      { key: 'count', label: 'Veces', type: 'number', placeholder: '2' },
    ],
    defaults: { count: 2 },
    summary: (p) => `¿Contactado > ${p['count'] ?? 2} veces?`,
  },
  {
    type: 'has_phone',
    kind: 'condition',
    label: 'Tiene teléfono',
    description: 'Tenemos teléfono guardado',
    icon: Phone,
    color: 'amber',
    group: 'Condición',
    fields: [],
    defaults: {},
    summary: () => '¿Tenemos su teléfono?',
  },
  {
    type: 'tone_is',
    kind: 'condition',
    label: 'Tono configurado es',
    description: 'El tono de la clínica coincide',
    icon: Palette,
    color: 'amber',
    group: 'Condición',
    fields: [
...
export const ACTIONS: BlockDef[] = [
  ...
  },
  {
    type: 'send_template',
    kind: 'action',
    label: 'Enviar plantilla configurada',
    description: 'Usa una plantilla de Seguimientos del menú de configuración',
    icon: FileText,
    color: 'sky',
    group: 'Comunicación',
    fields: [
      {
        ...
        type: 'select',
        options: [
          { value: 'pre_appt_24h',     label: 'Recordatorio 24h pre-cita' },
          { value: 'pre_appt_2h',      label: 'Recordatorio 2h pre-cita' },
          { value: 'post_appt_1h',     label: 'Post-visita (1h después)' },
          { value: 'post_appt_review', label: 'Pedido de reseña' },
          { value: 'cold_7d',          label: 'Followup en frío 7 días' },
          { value: 'cold_14d',         label: 'Followup en frío 14 días' },
          { value: 'cold_30d',         label: 'Followup en frío 30 días' },
          { value: 'reactivation',     label: 'Reactivación inactivos' },
        ],
        hint: 'Las plantillas se editan en Configuración → Seguimientos.',
      },
    ],
    defaults: { template_key: 'pre_appt_24h' },
    summary: (p) => `Plantilla: ${p['template_key'] ?? '—'}`,
  },
  {
    type: 'confirm_appointment',
    kind: 'action',
    label: 'Confirmar cita (fórmula)',
    description: 'Envía la fórmula exacta de confirmación de cita',
    icon: CalendarClock,
    color: 'sky',
    group: 'Conversión',
    fields: [],
    defaults: {},
    summary: () => 'Confirma cita con fórmula oficial',
  },
  {
    type: 'propose_slots',
    kind: 'action',
    label: 'Proponer 2 horarios',
    description: 'Sugiere 2 horarios concretos del próximo día abierto',
    icon: CalendarPlus,
    color: 'sky',
    group: 'Conversión',
    fields: [
      { key: 'duration_minutes', label: 'Duración cita (min)', type: 'number', placeholder: '60', optional: true },
    ],
    defaults: { duration_minutes: 60 },
    summary: () => 'Propone 2 horarios libres',
  },
  {
    type: 'pivot_back_to_goal',
    kind: 'action',
    label: 'Volver al objetivo',
    description: 'Responde breve a la desviación y vuelve a la cita',
    icon: Undo2,
    color: 'sky',
    group: 'Conversión',
    fields: [
      { key: 'brief_answer', label: 'Respuesta breve (opcional)', type: 'textarea', placeholder: 'Estamos en {{clinic_address}}.', optional: true, hint: 'Una línea como máximo. Después la IA vuelve a la cita.' },
    ],
    defaults: { brief_answer: '' },
    summary: () => 'Pivote a la cita',
  },
  {
    type: 'request_data',
    kind: 'action',
    label: 'Pedir datos del paciente',
    description: 'Solicita los datos formales para agendar',
    icon: ClipboardList,
    color: 'sky',
    group: 'Conversión',
    fields: [
      {
        ...
        label: 'Datos a pedir',
        type: 'tags',
        placeholder: 'nombre, apellido, telefono, motivo',
        hint: 'Separa con comas. Cada dato se pide en orden y se valida.',
      },
    ],
    defaults: { fields: ['nombre', 'apellido', 'telefono', 'motivo'] },
    summary: (p) => {
      const f = (p['fields'] as string[]) ?? []
      return f.length ? `Pide: ${f.slice(0, 3).join(', ')}${f.length > 3 ? '…' : ''}` : 'Pide datos'
    },
  },
  {
    type: 'wait_minutes',
    ...
    description: 'Pausa el flujo y reanuda más tarde',
    icon: Timer,
    color: 'violet',
    group: 'Control',
    fields: [
      { key: 'minutes', label: 'Minutos', type: 'number', placeholder: '30' },
    ],
    defaults: { minutes: 30 },
    summary: (p) => `Espera ${p['minutes'] ?? 0} min`,
  },
  {
    type: 'respect_quiet_hours',
    kind: 'action',
    label: 'Respetar quiet hours',
    description: 'Si estamos en quiet hours, posterga las acciones siguientes',
    icon: PauseCircle,
    color: 'violet',
    group: 'Control',
    fields: [],
    defaults: {},
    summary: () => 'Posterga si quiet hours activas',
  },
  {
    type: 'reinforce_persona',
    kind: 'action',
    label: 'Reforzar persona',
    description: 'Inyecta un addon de persona a la próxima respuesta IA',
    icon: UserCheck,
    color: 'violet',
    group: 'Comunicación',
    fields: [
      { key: 'persona_addon', label: 'Refuerzo de persona', type: 'textarea', placeholder: 'Recuerda: mensajes ≤ 35 palabras, una pregunta por turno, jamás digas que eres IA.', hint: 'Se concatena al prompt del doctor para esta conversación.' },
    ],
    defaults: { persona_addon: '' },
    summary: (p) => {
      const a = (p['persona_addon'] as string) ?? ''
      return a ? `Persona: ${a.slice(0, 30)}…` : 'Refuerza persona'
    },
  },
]
